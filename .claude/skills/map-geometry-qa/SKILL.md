...
that already shipped on this repo (`docs/DECISIONS.md`, entries 2026-07-19-a…h).
...
- **A gap already "covered" by a coarser neighbouring water polygon won't be
  touched by gap-first (by design — it doesn't override real water), even when
  the two land masses ARE meant to be contiguous** (Kyrenia↔Nicosia, ~0.038°:
  the midpoint sits inside the Mediterranean polygon, which reaches slightly
  further inland than the true coast). Bridge explicitly:
  `LineString([p1, p2]).buffer(gap/2 + 0.002, cap_style=2, join_style=2)`
  (flat caps, mitre joins — no round "pimples") unioned into the smaller side.
  **Do not use `convex_hull`** as the bridge — measured at ~0.18 deg² of
  spurious area for the same gap, grossly distorting the coastline; the buffered
  line only added 15.7 km².
...
names/owners on the live map, not a red test. This has now bitten **both**
Asia (2026-07-19-c, `--prefix ASI-`) **and** Europe (2026-07-19-h,
`--prefix EUR-`) independently — assume it can hit any continent and check
after any region-count change, not just the one you just edited.
...
- `docs/DECISIONS.md` 2026-07-19-a…h — the full incident history behind each rule.
