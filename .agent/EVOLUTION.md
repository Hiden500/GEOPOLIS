...
**Правило в `AGENTS.md` НЕ внесено — не помещается.** Устав занимает 16 345 из
16 384 байт порога (`normalized_size`, public eval:402): свободных 39, самой
плотной формулировке правила нужно 101. Поднимать порог запрещено, а уплотнять
общий устав — решение владельца, а не побочный эффект этой задачи. Пока
правила там нет, следующий агент напишет тот же однострочный проход: обёртка
существует, но НЕ предписана. Развилка возвращена пользователю.
...
**Decision:** keep — скрипт. Правило в `AGENTS.md` — pending human review:
без него обёртка доступна, но не обязательна.
