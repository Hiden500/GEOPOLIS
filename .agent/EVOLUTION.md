...
1. Наблюдение пользователя по нескольким последним картографическим задачам —
   сформулировано как устойчивый режим, а не единичный случай.
...
Layer changed: только `.claude/skills/map-geometry-design/SKILL.md`. Claude-local,
как соседний `map-geometry-qa`; канон в `.agents/skills/` не заводился, поэтому
обязательства побайтной парности не возникает.
...
Validation: public eval — 201 passed, 0 failed в дереве. Негативного контроля в
строгом смысле нет: скилл — инструкция, а не код, и «падать» ему не на чем.
