...
Дата / change id: 2026-08-07 / pr-only-merges

Problem evidence: истории `main` разошлись на живом репозитории. PR #16 был
влит на GitHub мерж-коммитом `eeeab8b` в `origin/main`, а локальный `main` за
то же время ушёл на 26 коммитов вперёд прямыми merge (`interface-rebuild`,
`roadmap-sync`, `gitignore-playwright`) без push. От общей базы `0187a8d`
получилось 6 коммитов только в origin и 26 только локально. Сведение стоило
отдельного дерева, конфликта в `docs/DECISIONS.md`, а сложенные журналы
перешагнули порог 2000 строк (2007) и потребовали архивации десяти записей.
Это не единичная оплошность: расхождение возникает всякий раз, когда часть
веток вливается через PR, а часть — локально, и обнаруживается только когда
`git merge --ff-only` внезапно отказывает.

Layer changed: одна строка в корневом `AGENTS.md`, раздел «Git, документация и
завершение». Самый узкий слой: правило уже жило там частично («push, merge …
требуют отдельного разрешения»), не хватало ЕДИНСТВЕННОГО канала влития.

Expected benefit: `origin/main` перестаёт отставать, `--ff-only` остаётся
рабочей проверкой «ветка готова», а CI прогоняется на каждой ветке до попадания
в `main`, а не после. Побочно: журналы перестают складываться скачками, и порог
чистки срабатывает от роста, а не от слияния историй.

Risks and containment: (1) PR на каждую мелочь дороже локального merge —
принято решением пользователя 2026-08-07, для однострочных правок остаётся
squash-мерж в интерфейсе GitHub; (2) правило не машинно-проверяемо: eval не
видит, каким способом коммит попал в `main`. Признак нарушения дешёвый и
известен — `git rev-list --left-right --count origin/main...main` с ненулевой
правой частью; (3) корневой `AGENTS.md` после правки в 19 байтах от лимита
16 KiB — следующая правка потребует сокращения, это отдельная задача.

Validation: правка текстовая, поведение агента ею и задаётся. Прогон
`run_public_evals.py` — 163 passed, 0 failed, включая проверку размера
корневого `AGENTS.md`.

Fresh-session status: держится самим `AGENTS.md` — он загружается в каждую
сессию, отдельного гейта не требует.
