...
- `scripts/map/AGENTS.md` — подробные map-правила по всем 4 пунктам;
- корневой `AGENTS.md` — явное описание `.agent/` (раньше папка была
  упомянута только косвенно, через 3 разрозненные ссылки);
- новый скилл `.claude/skills/find-existing-solutions/` — обязательный
  протокол поиска существующего решения ДО внешнего источника/новой
  реализации/вывода «не существует».

Expected benefit: прекращение повторов этих 4 классов ошибок без напоминания
пользователем; решения о курации остаются за человеком.
...
Fresh-session status: pending — срабатывание скилла подтвердится следующей
сессией.
