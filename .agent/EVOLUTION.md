...
- `.agents/skills/session-handoff/SKILL.md` (+ побайтное зеркало в
  `.claude/skills/`) — проверка признаков и формат пакета `.agent/handoff.md`;
...
Future-session gate: если за следующие сессии `.agent/handoff.md` ни разу не
пригодится при старте — правило избыточно и подлежит снятию, а не расширению.

Гейт и путь заменены записью 2026-08-08 `handoff-per-branch`: общий файл на все
сессии оказался неверным контейнером, пакет живёт в `.agent/handoff/<ветка>.md`.
...
- де-факто роль уже существовала без опоры: дерево `map-orchestrator` и пакет
  «оркестратор геометрии карты» в `.agent/handoff.md`;
...
Problem evidence — замеры организатора 2026-08-08 по `.agent/handoff.md`:
...
- `.agents/skills/session-handoff/SKILL.md` + побайтное зеркало в
  ...
  своей ветки;
- `.agent/handoff.md` удалён. Каталог `.agent/handoff/` не заводится пустым:
  путь создаётся первым обращением;
- `docs/TODO.md` — ссылка «подробности в `.agent/handoff.md`, пакет 3»
  заменена ссылкой на коммиты влитой ветки.
...
`.agent/handoff.md`: если за следующие сессии ни один файл в `.agent/handoff/`
не пригодится при старте новой сессии — правило избыточно и подлежит снятию, а
...
Problem evidence: перед удалением `.agent/handoff.md` прочитаны все четыре
...
Без этого шага удаление `.agent/handoff.md` было бы потерей знания, а не
уборкой.
...
деревьев и очереди открытых задач сознательно оставлены в git-истории удалённого
файла (`git show <коммит>^:.agent/handoff.md`).
