...
- [~] 8. **Приёмка.** `runCampaignWithLLM` на год; server tsc + npm test;
...
**Не выполнено.** Годовой прогон `runCampaignWithLLM`: живого провайдера нет
(LM Studio не запущен, `GEMINI_API_KEY` не задан). Готовая команда для сессии,
у которой провайдер есть, — в отчёте. Измерено, что МИР пускает эти приказы и
объясняет отказы кодами; НЕ измерено, что МОДЕЛЬ их выдаёт.
