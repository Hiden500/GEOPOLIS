...
## Beersheba/Jerusalem разрывы + Кирения (6-й округ Кипра) (2026-07-19-g)

Пользователь прислал ещё 3 скрина после -f: артефакты внутри Beersheba,
пропавшая часть Кипра ("посмотрел прошлую итерацию файла, там тоже нет —
где-то в скриптах значит"), разрывы у Jerusalem. Полная находка/фикс —
`docs/DECISIONS.md` "2026-07-19-g".

1. **Beersheba/Jerusalem — неверное допущение в `absorb_slivers`.** Ветка
   "1 земля + вода" пропускалась как "замаскировано фоном карты" —
   проверено прямым рендером конкретной ячейки на побережье Юж. Ливана:
   неверно, фон темнее моря, разрыв виден. Убрана проверка воды из этой
   ветки, остался только предохранитель по площади. +29 слайверов при
   пересборке Азии, все известные разрывы закрыты.
2. **Кирения — реальная дыра в `game_map.json`** (как Голан), не баг этой
   ветки: `build_europe_1946.py::KEEP_AS_IS["CY"]` был неверно помечен "5
   округов, не избыточно" (их 6). Источник — geoBoundaries CYP ADM1
   (тот же коммит, что ISR/PSE); Git LFS ловушка на raw-URL (нужен
   media.githubusercontent.com). Новый `build/extract_kyrenia.py`,
   добавлен в `FULL_REBUILD_STEPS` после `build_europe_1946.py`.
   Разрыв Кирения-Никосия (уже "покрыт" огрублённым морем, absorb
   корректно не трогает) закрыт минимальным буферным мостом
   (`cap_style=2`) — convex_hull отброшен (~0.18 deg² лишней площади).
3. **Ещё 2 независимых бага Кипра**, найдены при проверке Кирении:
   эксклав Larnaca (8.1 км², геометрически принадлежит Famagusta) и
   отдельная физическая дыра Famagusta↔Larnaca (~0.098°, вероятная
   причина "часть Кипра потерялось") — оба через
   `build_europe_1946.py::fix_cyprus_larnaca_exclave`/
   `fix_cyprus_famagusta_gap`, тот же приём минимального моста.

## Европа: names_ru.json/ownership_1946.json рассинхрон на полконтинента +
третья позиционно-хрупкая точка (2026-07-19-h)

Найдено попутно при проверке имени Кирении. Полная находка/фикс —
`docs/DECISIONS.md` "2026-07-19-h". Кратко: сдвиг на весь `names_ru.json`
от `EUR-0056` до конца списка, корень — НЕ эта сессия (Германия когда-то
раньше схлопнула зонную модель Берлина/Кильского канала/юга в современные
ADM1-юниты, 372→368 регионов, но названия ни разу не пересчитывались с
самого первого коммита; запись -c чинила только Азию). Починено
`remap_region_ids.py --prefix EUR- --apply` (0 неоднозначных) + вручную
добавлены 5 полностью новых записей (Baden-Württemberg/Berlin/Vatican/
Kosovo/Kyrenia) в names_ru.json и ownership_1946.json. Попутно найден и
исправлен баг самого `remap_region_ids.py` (`apply_overlay_remap` не
дропал неразрешённые старые ключи → осиротевшая запись Kiel Canal Zone
молча приписалась Ватикану на той же числовой позиции). Третья
позиционно-хрупкая точка (`CAPITAL_REGION_OVERRIDES` в
`generate_country_registry.py`, хардкод, не файл) подтверждена как уже
известная/отложенная категория — не новая, не трогается в этом заходе.

Верификация: тесты 12/12+9/9, tsc/vitest чисты, живой `/game/start` — все
368 EUR-регионов с именами, зоны оккупации Германии согласованы
(Baden-Württemberg→QGA, Berlin→QGS как окружающий Бранденбург), Кипр
согласован (Kyrenia/Larnaca/Famagusta/Nicosia→GBR).

Этим закрыт весь раунд геометрии/имён этой сессии. Осталось прежнее общее:
поле `tier`, единый проход по `CAPITAL_REGION_OVERRIDES` после всей карты,
Аральское море.

## Разворот: Кирения была диагностической ошибкой, откат Сирии/Иордании/
Ливана/Палестины на сырые провинции (2026-07-19-i)

Пользователь увидел кривой рендер Кипра, указал не проверять только по
`iso_a2` страны, и потребовал перенести из `game_map.json` сырые провинции
Сирии/Иордании/Ливана/Палестины(Израиля) без дальнейшего разбиения. Полная
находка/фикс/верификация — теперь в отдельном ExecPlan-файле по формату
`.agent/PLANS.md` (не в этом нарративном журнале, см. `AGENTS.md`
"Планирование и делегирование"): `.agent/plans/middle-east-raw-provinces-
revert.md`. Итог кратко: Northern Cyprus/Dhekelia найдены в самом
`game_map.json` (`iso_a2="-1"`), Кирения-графт и оба Кипр-моста этого дня
отменены как решавшие несуществующую проблему; SY/JO/LB/PS(IL) вернулись к
сырым провинциям источника (10→16, 6→12, 5→6, 15→8); Голан/UNDOF/An
Nabatiyah — сохранены как прежде, по решению пользователя. Полная история —
`docs/DECISIONS.md` "2026-07-19-i".
