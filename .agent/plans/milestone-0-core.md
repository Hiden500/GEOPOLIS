...
## Доработка A12 по независимому ревью закрытия аудита (read-only, 2026-07-26)

Ревью проводилось read-only на коммите `06aa730`; блокирующих находок нет,
подтверждённое (не трогалось) — палитра с роняющим её тестом, логика реформы на
пяти путях отказа, кризисный факт, бюджет накопления, капы, калибровка
коэффициентов. Мотивировка каждой правки — `docs/DECISIONS.md`, запись «Движок
примитивов: доводка по независимому ревью закрытия аудита».

| Правка | Файлы |
|---|---|
| `summary`: гранулярность правдивости — пара (цель, поле), а не поле; каждая пара из результата попадает в текст, нулевая — со словом `unchanged`; заголовки `repress` («(N unaffected)») и соседей `grant_autonomy` считаются от дельт, а не от состояния | `primitives/PrimitiveEngine.ts` (`describeImpacts`, `untouchedNote`) |
| Рантайм-сверка: сравнение ВЕЛИЧИН в обе стороны вместо набора ключей в одну; ловит скрытый, выдуманный и подменённый по величине эффект; допуск `IMPACT_REPORT_EPSILON = 1e-9` | `primitives/PrimitiveEngine.ts` (`findMisreportedImpacts`, `totalsByKey`) |
| Три теста, доказывающих, что сверка кусается: ломается отчёт командного слоя (`vi.spyOn` на `addGroupImpact`), эффект применяется — каждый требует отката целиком | `primitives/__tests__/PrimitiveEngine.test.ts` |
| Локализация текста промта: `groupId` и `region <id>` → `LocalizedText` через `getText(..., LLM_LOCALE)` с фолбэком на идентификатор | `primitives/PrimitiveEngine.ts` (`groupLabel`, `regionLabelById`) |
| Четыре комментария приведены к коду: граница сверки (канал памяти, не состояние целиком) ×2, достижимость ветки отказа платежа, фактическое состояние дипломатии 1946 | `primitives/PrimitiveEngine.ts`, `primitives/types.ts` |
| Тест порога восстания: `Math.max` по всем размеченным регионам вместо одного 187; «самый напряжённый регион» вычисляется, а не зашит числом | `primitives/__tests__/PrimitiveEngine.test.ts` |

Текст на кейсе рецензента (`repress` региона 187 целиком, доминант
`lithuanians` 0.94 на потолке обоих полей) — фактический вывод после правки:

```text
Security forces moved against 4 group(s) in Šiauliai (1 unaffected):
suppression unchanged for Lithuanians (1.000), +0.420 for Russians (0.000 → 0.420),
+0.424 for Latvians (0.000 → 0.424), +0.424 for Jews (0.000 → 0.424);
alienation unchanged for Lithuanians (1.000), +0.111 for Russians (0.000 → 0.111),
+0.110 for Latvians (0.000 → 0.110), +0.110 for Jews (0.000 → 0.110)
```

Прямой подсчёт по данным 1946 (основание правки комментария и записи в
`docs/TODO.md`): 157 стран, **0** с непустым `diplomacy.allies`, **0** с
непустым `relations`, 14 с `puppets`, 14 с `sphereOfInfluence`. Регионов 1399,
из них с демо-разметкой — 14; максимум недовольства 0.6320 (регион 187), далее
0.6243 (68 и 69), 0.6152 (70), 0.6131 (186), 0.6064 (185) при пороге восстания
0.65 и пороге инцидента 0.40.

Каждая новая проверка предъявлена как кусающаяся временным откатом
реализации: возврат `describeImpacts` к гранулярности поля роняет 5 тестов
(4 параметризованных + кейс 1946), возврат сверки к сравнению ключей роняет 2
(выдуманный эффект и подменённая величина) и оставляет зелёным тест скрытого
эффекта — то есть ровно ту половину, которую старый механизм и покрывал.

Верификация A12: `server tsc` — 0 ошибок; `server npm test` — 54 файла,
**873 passed**, 1 skipped, 0 failed (baseline `06aa730`: 54 файла, 864 passed).
Agent-конфигурация не менялась, public eval не требуется.

Осознанно НЕ сделано в A12: idempotency-ключ и кризисный кап ≤5 (следующая
сессия); рантайм-сверка не расширена на координаты идеологии / политическую цену
/ объект карты — граница названа в `docs/PRIMITIVES.md` §4 и `docs/TODO.md`;
стартовая дипломатия 1946 не наполнена (данные, не движок).
