...
- [x] A8 доработка по второму независимому ревью (2026-07-26): кап «один verb на цель за
      ...
      теста пересечения
- [x] A9 доработка по третьему независимому ревью (2026-07-26): кап накопления следа;
      нулевая уступка — нулевой отклик; цель реформы = источник (см. раздел ниже)
- [x] A10 демо-срез 1946 наполнен внешне; `campaignSmoke` переведён с расстановки регионов
      на инвариант механики (коммит `ed12661`, предыдущая сессия)
- [x] A11 доработка по ВНЕШНЕМУ независимому аудиту (Codex, read-only, коммит `755d641`):
      структурированный результат примитива вместо скаляра `magnitude`; палитра видит
      появление/исчезновение нулевых полей; предпосылки по виду инцидента; reject
      недостижимой реформы до списания цены; кризисный факт называет индекс индексом
...
## Доработка A11 по внешнему независимому аудиту (Codex, read-only, 2026-07-26)

Аудит проводился read-only на коммите `755d641`. Критичных находок нет;
подтверждённое (не трогалось) — блокировка числовых параметров схемой,
зависимость всех каналов магнитуды от состояния, кап verb/цель, бюджет
накопления с откатом целиком, рантайм-палитра, вывод недовольства из
геометрии/экономики/памяти, различимость веток. Мотивировка каждой правки —
`docs/DECISIONS.md`, запись «Движок примитивов: правки по внешнему
независимому аудиту».

| Правка | Файлы |
|---|---|
| Результат примитива: discriminated union по глаголу вместо скаляра `magnitude` + заготовленной строки; `before`/`after`/`delta` по каждому полю и цели, отдельно `politicalCost`, `neighbourEffects`, `mapFeatureId`, `disputedWithCountryId`; `summary` выводится из дельт | `primitives/types.ts`, `primitives/PrimitiveEngine.ts` |
| Рантайм-сверка «отчёт полон»: диф памяти против опубликованных эффектов, примитив с неотчитанным эффектом откатывается | `primitives/PrimitiveEngine.ts` (`impactDeltas` теперь знаковый, `impactEffectsOf`) |
| Палитра видит появление/исчезновение поля: строгое сравнение, поблажка сужена до структурных нулей нового/удалённого контейнера | `primitives/statePaths.ts` + новый `__tests__/statePaths.test.ts` |
| Предпосылки по виду инцидента: порог восстания выше кризисного; спор требует чужой границы с не-союзником | `primitives/PrimitiveEngine.ts`, `shared/defines/discontent.ts` (`SPAWN_INCIDENT_UPRISING_MIN_DISCONTENT`) |
| Реформа: reject до списания цены, если сдвиг недостижим; шаг считает одна функция для validate и apply | `primitives/PrimitiveEngine.ts` (`reformStep`, `reformAxes`, `unreachableAxes`) |
| Кризисный факт: индекс назван индексом, а не долей населения | `simulation/politics/DiscontentTick.ts` + тест |
| Однородный дефект в остальных фактах: `Country.name` (`LocalizedText`) подставлялся в шаблон напрямую → `[object Object]` в промте | `simulation/SimulationEngine.ts` (4 факта), `simulation/ObjectiveTick.ts` (1 факт) |
| `branchDivergence`: `\|зазор\| < 0.02` больше не называется пересечением; печатается знак | `primitives/__tests__/branchDivergence.test.ts` |

Замер зазора веток (текущая калибровка, `discontent(grant_autonomy) −
discontent(enact_reform)` в целевом регионе): месяц 36 → −0.0026889, 37 →
−0.0015271, 38 → −0.0004118, **39 → +0.0006590**. Смена знака между 38-м и
39-м месяцами, то есть «пересечение около 36-го» в прежнем комментарии было
неточным: на 36-м кривые лишь ближе порога различимости.

Прежняя логика палитры воспроизведена отдельно и подтверждает находку:
`collectChangedPaths({}, { unauthorizedNumericField: 0 })`,
обратное удаление и вложенный `visibleAtZoom: 0` — все три давали `[]`.

Достижимость порога восстания на реальных данных проверена отдельно: самый
напряжённый размеченный регион 1946 (187) стоит на **0.632**, то есть ниже
порога 0.65 с первого месяца — восстание не выдаётся сразу, к нему надо
подвести мир. Документированная §4 цепочка `incite_unrest(severe) →
spawn_incident(uprising)` по региону 187 поднимает недовольство до 0.728 и
проходит целиком (предпосылка второго считается по состоянию после первого).
Закреплено тестом на данных сценария, а не на фикстуре: он падает и если
калибровка загонит порог выше достижимого, и если разметка датасета уедет.

Верификация A11: `server tsc` — 0 ошибок; `server npm test` — 54 файла,
**864 passed**, 1 skipped, 0 failed (baseline `ed12661`: 53 файла, 838 passed);
`client tsc` — 0 ошибок; `python scripts/map/validate_demographics_1946.py` — OK
(9 групп, 14/1399 регионов, 7/157 стран); `python scripts/map/test_validate_demographics_1946.py` —
15 тестов OK.

Осознанно НЕ сделано в A11: idempotency-ключ и кризисный кап ≤5 (вне scope,
следующая сессия); `?? 0` в двух оставшихся местах чтения `applied` — сужено и
переописано в `docs/TODO.md`, но не закрыто.
