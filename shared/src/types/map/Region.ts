...
export interface Region {
  ...
  /**
   * Сухопутные соседи. Имя говорит «land» намеренно (К-6, 2026-08-20): пока
   * воды в графе не было, поле звалось `neighboringRegionIds` и молча означало
   * «все соседи». С появлением водных узлов такое имя стало ложью, а
   * переименование — механизмом: потребитель обязан выбрать сушу, воду или обе,
   * и не может остаться на прежнем поведении по невнимательности.
   */
  landNeighboringRegionIds: number[];

  /**
   * Водные узлы, которых регион КАСАЕТСЯ берегом (`waters.json`). Достижимость
   * по воде здесь не хранится — это запрос к графу; транзитивное замыкание
   * раздуло бы сценарий и завело полный обход мира.
   *
   * Сегодня поле не читает никто: перевод конкретных потребителей на воду —
   * решение домена `logic` (задание T-17), а не следствие появления данных.
   */
  adjacentWaterIds: number[];
