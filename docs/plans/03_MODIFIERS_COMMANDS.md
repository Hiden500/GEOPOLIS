...
- [x] Промт LLM может включать значения defines (хотя бы капы действий) из того же источника, что использует движок. — уже было выполнено планом 02 (`shared/src/defines/llmActionCaps.ts` — файл удалён 2026-08-02 вместе с каналом `actions`, капы переехали в `research.ts`/`military.ts`/`resources.ts`), этим планом не тронуто.
...
Формат — TypeScript-файлы по подсистеме (паттерн `defines/research.ts`), не
