...
Карта остаётся полноэкранной основой интерфейса. Поверх неё лежит один экран —
`client/src/game/Screen.tsx`: приклеенная к краю рама (ШАПКА и ХОД) и
всплывающие поверхности (ЯЩИК с ТОМАМИ, ЛЕНТА событий, ЛИСТ приказов, ПОЛОСА
региона, РЕЖИМЫ карты, окно РЕЕСТРА). Экран не знает ни про `GameState`, ни про
сеть: он получает модель (`client/src/game/model.ts`) и набор действий, а
собирает их `client/src/game/GameShell.tsx` — единственное место, где сходятся
состояние партии, MapLibre и вызовы API. Второй источник той же модели —
песочница `?showcase=screen` на шаблонных данных, поэтому форма экрана не может
разойтись между игрой и макетом.

Слой `client/src/hud/` и `client/src/primitives/` удалён 2026-08-07 вместе с
`GameView`. Актуальная композиция и правила адаптивности описаны в
`docs/UI_DESIGN.md`.

**Предложено, не реализовано** (в этом worktree поля `GameState.lastTurnReport` в коде нет —
grep пуст; реализация жила на невлитой bootstrap-линии): обратная связь хода — опциональный
diff ключевых показателей игрока за ход + id завершённых целей для «национального пульса».
Не описывать как готовое; целевую роль обратной связи хода см. `CONCEPT.md`.
...
**Текущий статус реализации**: ручной LLM-цикл работает целиком — `GET /llm/prompt` отдаёт промт, окно «Ручной цикл ИИ-режиссёра» (открывается из МЕНЮ, живёт в `client/src/game/Screen.tsx`) копирует его в буфер обмена и предоставляет поле вставки ответа, `POST /llm/response` принимает и применяет ответ. Дополнительно есть автоматизированный путь через `GeminiProvider` (`POST /llm/auto`), не требующий ручного копирования. Ход защищён гейтом: `GameState.llmRespondedThisTurn` — `GameService.advanceMonth` бросает `LLMGateError`/409, если LLM не ответила за цикл. Длина хода переменная (1–12 месяцев): `advanceTurnSchema`, параметр `months` в `POST /game/next-turn`. Подробности и история решений — `docs/DECISIONS.md`.
...
* тысячи регионов;
* сотни государств;
* объекты карты (уникальные хранятся, их немного; массовое количественное — производные слои раскраски, не хранимые объекты — `CONCEPT.md` §5.6).
