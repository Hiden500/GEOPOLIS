...
Ещё не обработанная Океания временно сохраняет прежнее объединение в четырёх
блоках `QCG/QCF/QCU/QCZ`. `country_merge.json` — generated-результат
...
На текущем срезе завершены Южная Америка, Северная Америка/Карибы, Европа,
Африка и Азия.
