...
ДЕФЕКТ 1: Далянь потерял 1 424,2 км². Далянь ушёл с 4 частей / 3 540,7 км²
до 1 части / 2 116,6 км², а Ляонин прибавил ровно столько же
(55 984,0 -> 57 408,2) и вырос с 4 частей до 21. Совпадение точное — это
перенос, не потеря. Из 1 424,2 км² только 271,7 — острова; крупнейший кусок
1 152,5 км² — срез суши Ляодунского полуострова, сваренный с материковой
частью Ляонина, то есть сдвинулась и сухопутная граница.

ВИНОВНИК — `628a3bd` (2026-07-27, «restore Panama Canal Zone, fix Kiel Canal
Zone stray fragment»), а НЕ `a9530ea` («топологическая пересборка»), как
утверждала эта докстрока до 2026-08-08. `a9530ea` изменил 18 регионов на
110,8 км² суммарно, и Даляня среди них нет. Тем же коммитом `628a3bd` откачена
вся курация Китая от 23.07 — Циндао, Шаньдун и пять маньчжурских регионов;
их возвращает `build/restore_china_curation_master.py`, а сторожит от
повторения `build/verify_china_curation.py` в STANDARD_STEPS.

Прослеживать такое можно ТОЛЬКО по `git rev-list HEAD -- <файл>`: `git log`
смешивает ветки по дате, `a9530ea` и `628a3bd` не предки друг друга, и
«бисект» по его выводу даёт бессмысленный ответ.
