...
def absorb_slivers(mutable_feats, context_geoms=(), water_geoms=(),
    ...
    for cell in cells:
        ...
        if len(land_touch) < 2:
            ...
            # "пинхол" без воды рядом. Предохранитель — площадь: не
            # поглощаем то, что похоже на целое море/озеро.
            #
            # ИСПРАВЛЕНО (2026-07-23, живые рендеры пользователя — видимые
            # разрывы вдоль берега New York-Genesee/Lake Erie, западного
            # берега Michigan, южного берега Lake Superior у Marquette):
            # предохранитель ошибочно использовал ГОЛЫЙ MAX_COMPACT_AREA
            # без ribbon-исключения, хотя длинные прибрежные разрывы у 1
            # суши — САМЫЙ частый случай ribbon-формы (длинный тонкий разрыв
            # вдоль берега озера, где нет второй суши-соседа с другой
            # стороны). Найденная ячейка (New York — Genesee/Erie,
            # 114.7 км2, compactness=0.031 — явная лента) проходила общий
            # ribbon-тест выше, но отклонялась именно здесь голым порогом
            # 0.008 deg2 (~70-90 км2) без учёта формы. Тот же ribbon-тест,
            # что уже используется для общей классификации (area/
            # compactness), применяется и тут — не отдельный, более мягкий
            # порог, а СИММЕТРИЧНОЕ решение с уже принятым общим правилом.
            if cell.area > MAX_COMPACT_AREA and not (
                    cell.area <= MAX_RIBBON_AREA and c < RIBBON_COMPACTNESS):
                continue
