...
Защиты:
  ...
  - Ячейка должна касаться >=2 РАЗНЫХ земельных фич — прибрежная полоса
    между землёй и морем (1 земля) не трогается: этот класс щелей
    маскируется фоновым цветом карты, не геометрией.
...
# низкий Polsby-Popper, неотличимый от ленты зазора. Прим.: Кинерет СЕЙЧАС
# не дыра (округ Kinneret в geoBoundaries ISR включает воду озера, Tiberias
# покрывает его с первой пересборки Палестины) — точка здесь на случай
# смены источника или выреза озера в отдельную LAK-фичу в будущем.
PROTECTED_HOLE_POINTS = [
    (35.59, 32.82),   # Кинерет (Галилейское море) — на стыке Палестины и Сирии
]
...
def absorb_slivers(mutable_feats, context_geoms=(), water_geoms=(),
    ...
    for cell in cells:
        ...
        if not land_touch:
            continue
        water_touch = any(cell.boundary.intersection(w.boundary).length > 1e-9
                          for w in water_geoms)
        if len(land_touch) < 2:
            # Одна земля: прибрежная полоса (сосед — море) не трогается,
            # этот класс щелей маскируется фоном карты. Но "пинхол" —
            # микроячейка у границы единственной фичи БЕЗ воды рядом
            # (порождается изломами шва) — поглощается в неё же.
            if water_touch or cell.area > MAX_COMPACT_AREA:
                continue
