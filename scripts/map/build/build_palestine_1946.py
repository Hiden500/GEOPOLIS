...
FULL_REBUILD_STEPS, build_asia_1946.py больше не читает выходной файл этого
скрипта). Пользователь потребовал откатить эту реконструкцию на сырые
провинции game_map.json (6 округов Израиля + 2 пятна Палестины, без
дальнейшего разбиения) — решения о числе/составе регионов принимает
пользователь явным запросом, не агент инициативой. Скрипт и его выходной
файл (`palestine_1946_historical.json`) оставлены на диске как справочный
материал — группировка ADM2→исторический подрайон ниже пригодится, когда
пользователь укажет, какие именно провинции разбивать дальше. См.
docs/DECISIONS.md за полной историей разворота.
