...
СЛОЙ ДРЕЙФУЕТ ОТ ПЕРЕНОСА К ПЕРЕНОСУ. Замер 2026-08-08: слой написан под мир
из 1399 регионов (тогда это был коммит `1b67bcd`), где ложится идеально —
id 900 = Campeche и майя, 1100 = Córdoba и италоаргентинцы, 1300 = Masvingo и
шона, 1399 = Pohnpei и микронезийцы. Позже мир вырос до 1444, и те же числа
стали означать другие регионы: id 1300 — уже камерунский «Nord» с
зимбабвийским составом, id 1399 — Южная Австралия с микронезийским.

ОПОРА — ПОСЛЕДНЕЕ СОСТОЯНИЕ, ГДЕ СЛОЙ ДОКАЗАНО СХОДИТСЯ, А НЕ ДАТА КОММИТА.
Правило «опора — коммит, под который слой писался» продержалось ровно до
следующего переноса: перенос 2026-08-08 сам стал новым «писался под». Опора
2026-08-08 (тот самый `1b67bcd`) была верной ОДИН перенос и устарела уже к
2026-09-04 — прогон с ней дал мусор (Уругвай получил «хуту 0.82», Южная
Америка не сопоставилась почти целиком), при том что `main` на 2026-09-04
сходится идеально по тем же именам. Сходимость проверяется СВЕРКОЙ ИМЁН
РЕГИОНОВ С СОСТАВОМ (образец — Гомель → белорусы, Чьяпас → майя, Понпеи →
микронезийцы), а не тем, что коммит когда-то был local последним. Каждый
успешный перенос делает СЕБЯ новой опорой для следующего — обновляй этот
абзац и пример коммита при каждом переносе, иначе следующий агент повторит
ту же ошибку с уже устаревшим `1b67bcd`.
...
Вход:  --ref-world, --ref-core, --ref-state — снимки ПОСЛЕДНЕГО состояния, где
       слой ДОКАЗАНО сходится с составом карты (сверкой имён, не датой
       коммита) — на 2026-09-04 это `main`, а не `1b67bcd` (устарел, см. выше);
