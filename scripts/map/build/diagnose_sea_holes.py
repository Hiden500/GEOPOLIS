...
diagnose_missing_land.py/diagnose_scattered_regions.py). Находит класс
пропаж, который diagnose_missing_land.py НЕ ловит: дыры-острова, вырезанные
внутри полигонов морей/озёр (interior rings), для которых на выходе нет
покрывающей суши. В отличие от diagnose_missing_land.py (сверяет raw
