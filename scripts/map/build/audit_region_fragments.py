...
def classify(part_owner, main_owner, members, wfrac):
    ...
    # Единица сырья без имени — не спор, а отсутствие разметки (замер
    # 2026-08-30): все 72 находки этого рода — антарктические сектора
    # (ANT-0001/0003/0004/0007), у которых сырьё не размечено по ADM1
    # вообще. Прежний фикс добавлял в `members` только единицу под ГЛАВНЫМ
    # телом региона (см. вызов classify()) — это закрыло Albany/Ponta
    # Porã/San Juan, но не Антарктиду: её острова лежат под ДРУГИМИ
    # безымянными сырыми полигонами, не тем же, что главное тело, и до
    # `members` не дотягиваются. ISO уже сверен выше (та же страна), значит
    # оспаривать с безымянной единицей нечего — кусок идёт на обычную
    # проверку остров/эксклав, а не молча в «спорные».
    if p_idx not in members and p_name:
        return ("foreign_adm1",
