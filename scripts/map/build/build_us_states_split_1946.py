...
def main():
    ...
    # ВАЖНО (2026-07-29): обрезка ниже — только по СОСЕДЯМ (Канада/Мексика),
    # но НЕ по воде. Юридические границы округов США идут ПО АКВАТОРИИ
    # (San Juan County владеет водами пролива Хуан-де-Фука, Accomack —
    # Чесапикского залива), поэтому вода попадает сюда как СУША. Отдельный
    # шаг `clip_us_counties_to_raw_land.py` (следующий в FULL_REBUILD_STEPS)
    # обрезает county-кластеры по сырой суше game_map.json — вынесен в
    # отдельный скрипт, а не встроен здесь, потому что нужен порог по доле
    # (у 5 регионов акватория 11-96%, у остальных расхождение оцифровки
    # <1%, и обрезка последних огрубила бы берег). Без этого шага
    # `clip_sea_by_land.py` резал МОРЕ по ложной суше и разбил
    # North Pacific — American Sector на 17 кусков, см. docs/DECISIONS.md
    # 2026-07-29.
    #
