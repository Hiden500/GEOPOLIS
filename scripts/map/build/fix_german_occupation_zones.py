...
ГЕОМЕТРИЮ И `area_km2` НЕ ТРОГАЕТ — и потому их не пересчитывает (проверено
2026-08-10, T-8). Мастер здесь только ЧИТАЕТСЯ, и только два свойства: `name`
(чтобы найти регион) и `region_id` (чтобы записать его в другие файлы).
Записываются `out/ownership_1946.json` и `config/occupation_overlay.json` —
владение, а не полигоны. Пересчёт площади без правки полигона был бы шумом.

Появится здесь правка `feature["geometry"]` — в том же проходе обязана
появиться и строка `props["area_km2"] = round(area_km2(geom), 1)`: площадь
уезжает в `regions.core.json.area` и дальше в экономику региона. Сторожит это
`verify_area_matches_geometry.py` в `make_1946.py`, а не память автора.
