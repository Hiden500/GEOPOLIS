...
STANDARD_STEPS = [
    ...
    # Морская смежность (К-6): инварианты формы, которую только что записал
    # import_to_game.py — симметрия, разрешённость id, непересечение пространств
    # суши и воды, отсутствие петель, и головное «регион без сухопутных соседей
    # обязан иметь берег». Стоит здесь же, сразу за контрактом полей, по тому
    # же принципу: сначала проверяем ФОРМУ выхода импорта, и только потом
    # строим по нему реестр стран и экономику.
    "validate_sea_adjacency_1946.py",
