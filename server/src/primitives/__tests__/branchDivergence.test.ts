...
describe("ветки не схлопываются: три ответа игрока → разное состояние", () => {
  ...
  /**
   * НАХОДКА КАЛИБРОВКИ (2026-07-26), а не косметика: уступка затухает
   * (CONCESSION_DECAY_RATE), реформа постоянна — поэтому кривые недовольства
   * целевого региона у grant_autonomy и enact_reform ПЕРЕСЕКАЮТСЯ примерно на
   * 36-м месяце. Ветки при этом остаются разными состояниями мира (координаты
   * власти сдвинуты навсегда), но по одному скаляру в этот момент неразличимы.
   * Тест фиксирует пересечение явно, чтобы будущая калибровка коэффициентов
   * увидела его как известное свойство, а не как новый баг.
   */
  it("известное пересечение: grant_autonomy догоняет enact_reform по недовольству к 36-му месяцу", () => {
    ...
    expect(Math.abs(autonomy.target - reform.target)).toBeLessThan(DISTINGUISHABLE_DELTA);
    // Но сами состояния мира разные: реформа сдвинула координаты власти.
