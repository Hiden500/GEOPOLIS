...
describe("ветки не схлопываются: три ответа игрока → разное состояние", () => {
   ...
   * целевого региона у `grant_autonomy` и `enact_reform` сближаются и примерно
   * к 36-му месяцу пересекаются.
   *
   * Само пересечение тест НЕ утверждает, и это осознанно. Его точка —
   * производная от плейсхолдерных коэффициентов (`docs/TODO.md`, «Калибровка
   * коридоров магнитуды»): любая калибровка её сдвинет, и жёсткое
   ...
   * сближения по одному скаляру. Сама величина зазора печатается, чтобы будущая
   * калибровка видела число, а не гадала.
   */
  it("сближение по одному скаляру не делает состояния мира одинаковыми", () => {
    ...
    const scalarGap = Math.abs(autonomy.target - reform.target);
    // eslint-disable-next-line no-console
    console.info(
      `[калибровка] 36-й месяц: |discontent(grant_autonomy) − discontent(enact_reform)| = ` +
      `${scalarGap.toFixed(4)} (порог различимости ${DISTINGUISHABLE_DELTA}; ` +
      `${scalarGap < DISTINGUISHABLE_DELTA ? "кривые пересеклись" : "ещё не пересеклись"})`
    );
