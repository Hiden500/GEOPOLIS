...
describe("бюджетные глаголы не знают о ранге страны", () => {
  it("страна ротации (не-major) двигает и исследования, и производство", () => {
    // Свойство, доказанное замером 2026-08-02 (`docs/DECISIONS.md`, коммит
    // `35ca124`): ограничение «сдвиги фокуса только у Major Power» жило ТОЛЬКО
    // в тексте промта, движок про `tier` не знает вовсе — и снятие запрета
    // дало 0 -> 26 применений у 19 не-мажоров за 72 хода.
    //
    // Сторож нужен ИМЕННО отдельным: остальные тесты файла держат это свойство
    // молча (фикстура даёт `tier: "minor"` по умолчанию), а молчаливая
    // гарантия исчезает от одной правки фикстуры. Здесь ранг задан явно и
    // назван — проверка на стороне ДВИЖКА, в дополнение к сторожу текста
    // промта в `LLMService.test.ts`, который тексту и принадлежит.
    const state = game();
    const usa = usaOf(state);
    usa.tier = "minor";
    expect(usa.tier).not.toBe("major");

    const research = apply(state, researchShift({ intensity: "severe" }));
    const production = apply(state, {
    ...
    expect(research.rejected).toEqual([]);
    expect(production.rejected).toEqual([]);
    expect(usaOf(state).technology.researchAllocation![DOMAIN]!).toBeGreaterThan(0);
    expect(usaOf(state).military.productionAllocation!.tanks!).toBeGreaterThan(0);
  });
});
...
describe("research_shift: долю считает движок, а не модель", () => {
  ...
  it("ПРЕДОХРАНИТЕЛЬ: домен, которого у страны нет, отклоняется и записи не создаёт", () => {
    ...
    // Текст отказа идёт В СЛЕДУЮЩИЙ ПРОМТ и обязан учить модель («чтобы не
    // долбилась в невозможное», `docs/PRIMITIVES.md` §3). Отсылать его к
    // «Technology line страны» нельзя: та печатает только домены с прогрессом
    // (`LLMService.getTechTierSummary`), а у стран ротации её нет вовсе —
    // ровно тот неполный список, из-за которого 8 из 11 отказов замера
    // 2026-08-02 были выдуманными именами доменов.
    const text = rejectionPromptText(result.rejected[0]!.rejection);
    expect(text).toContain("psionics");
    expect(text).not.toContain("Technology line");
  });
