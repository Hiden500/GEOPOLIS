...
const HINTED_CHANNELS: HintedChannel[] = [
  ...
  {
    stem: "REPRESS_LEGITIMACY_COST",
    name: "repress · цена мандата (legitimacy = 0 — терять уже нечего)",
    min: REPRESS_LEGITIMACY_COST_MIN,
    max: REPRESS_LEGITIMACY_COST_MAX,
    collapsed: () => repressLegitimacyCostFactor(1, 0),
    open: () => repressLegitimacyCostFactor(1, COUNTRY_POLITICS_SCALE_MAX / 2),
  },
  {
