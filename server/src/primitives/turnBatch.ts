...
function placeHistoryEntries(
  ...
  if (
    ...
    // Подчинение места не имеет вовсе: оно меняет положение ГОСУДАРСТВА, а не
    // то, что происходит на земле. Поглощение места имеет — регионы меняют
    // флаг, — но записи не получает, и это то же решение, что у мира с
    // аннексией по договору: смена владельца уже выражена самим владением
    // региона, а строка отклика у обоих называет страны, а не место. Завести
    // исключение для `annex` там, где `peace` двигает границу тем же способом
    // и записи не имеет, значило бы дать два разных ответа на один вопрос.
    // Общий след «здесь сменился флаг» — одной работой на оба (`docs/TODO.md`).
    applied.verb === "puppet" ||
    applied.verb === "annex" ||
    ...
    // Воздействия, переехавшие из старого канала, делятся по тому же признаку.
    // Гарантия — обязательство МЕЖДУ государствами, места не имеет. Сдвиги
    // бюджетного фокуса — общегосударственные акты без места, ровно как
    // реформа: приписать «перевели исследования на авиацию» одному региону
    // значило бы выдумать факт, которого в результате нет.
    applied.verb === "guarantee" ||
    applied.verb === "research_shift" ||
    applied.verb === "production_shift"
  ) {
    return entries;
  }

  // `build_extraction` в список исключений НЕ входит: стройка происходит в
  // конкретном регионе, и «здесь построили добычу» принадлежит истории места
  // так же буквально, как отток капитала.
