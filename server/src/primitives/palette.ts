...
export const PRIMITIVE_PALETTE: Record<PrimitiveVerb, readonly string[]> = {
  ...
  // Гарантия: запись в список обязательств + потепление пары. Ни влияния, ни
  // санкций, ни союзов — обещание защитить не должно уметь молча создать второе
  // обязательство. Элементы списка объявлены `[*]`/`[+]` по той же причине, что
  // у `puppet`: у `string[]` стабильной идентичности нет, и появление первого
  // элемента диф читает как изменение позиции 0.
  guarantee: [
    "countries[*].diplomacy.guarantees[*]",
    "countries[*].diplomacy.guarantees[+]",
    "countries[*].diplomacy.relations.{*}",
  ...
  // Сдвиг фокуса исследований: РОВНО словарь долей по доменам. Ни казны, ни
  // прогресса доменов — примитив меняет, КУДА пойдут деньги следующим тиком, а
  // не сколько их и не что уже изучено. Прогресс считает `ResearchTick`.
  research_shift: [
    "countries[*].technology.researchAllocation.{*}",
  ],

  // Сдвиг фокуса производства — то же самое для категорий техники. Самой
  // техники (`military.equipment`) в палитре нет намеренно: её растит
  // `MilitaryTick` из долей, и разрешить примитиву писать туда значило бы дать
  // ему создавать танки приказом.
  production_shift: [
    "countries[*].military.productionAllocation.{*}",
  ],

  // Добывающие мощности: уровень в регионе и казна того, кто за него платит.
  // Ни депозитов, ни добытого сырья, ни владения регионом — строится мощность,
  // а не месторождение.
  build_extraction: [
    "regions[*].extraction.{*}",
    "countries[*].economy.treasury",
