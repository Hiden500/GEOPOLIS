...
export class WarService {
  ...
  declareWar(initiatorId: string, targetId: string, warGoal?: string): War {
    // Существование сторон проверяется ЗДЕСЬ, а не только в обёртке команды
    // (добавлено Милстоуном 1, сессия жизненного цикла).
    //
    // До этого защита стояла ровно в одном месте — `commands/war.ts`, — и
    // держалась тем, что оба сегодняшних вызывающих идут через него. Пока
    // страны не исчезали, обойти её было неоткуда. С жизненным циклом
    // (`CONCEPT.md` §7.1) страна становится исчезающей сущностью, и война с
    // идентификатором-призраком — не падение, а тихо неверное состояние: она
    // попадёт в `wars`, в промт и в проверку висячих ссылок как настоящая.
    //
    // Бросок, а не `undefined`: сигнатура обещает войну, и все вызывающие
    // проверяют вход раньше. Дойти сюда можно только собранным мимо них
    // вызовом — то есть ошибкой кода, которую громкий отказ показывает, а
    // тихий `undefined` спрятал бы в следующую подсистему.
    for (const id of [initiatorId, targetId]) {
      if (!this.game.countries.some(c => c.id === id)) {
        throw new Error(`Cannot declare a war with a country that does not exist: ${id}`);
      }
    }
