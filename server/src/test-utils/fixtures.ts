...
export function createTestCountry(overrides: Partial<Country> = {}): Country {
  return {
    ...
    goals: [],
    // Нейтральный темперамент по умолчанию (docs/AI_RULES.md). Нейтральный —
    // значит `traitScale(1, …) === 1` при любой силе, то есть пороги Правил
    // A/B/C у этой фикстуры равны общим константам. Тесты, которым нужна
    // вариативность характера, переопределяют поля явно
    // (`AiBehaviorTick.test.ts`, блок про aiTraits).
