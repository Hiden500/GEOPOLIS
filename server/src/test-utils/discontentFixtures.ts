 ...
 *
 * Числа подобраны так, чтобы все регионы стояли ровно на ПАРИТЕТЕ со своей
 * страной (ВВП на душу региона = ВВП на душу страны, `WELFARE_PARITY`):
 * экономический член формулы обнуляется, и тест меряет ровно геометрию
 * дистанции, а не смесь. Именно поэтому мини-мир не видел ни односторонности
 * благосостояния, ни недостижимых порогов — их ловят guard-тесты на живых
 * данных (`server/src/simulation/politics/discontentLiveGuards.test.ts`).
 */
