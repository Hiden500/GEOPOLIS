...
export function ShapedBar({
   ...
   */
  /*
   * НИЖНЯЯ кромка полосы встаёт на границу колонки ФЛАГА, верхняя уходит
   * правее на размах. Наоборот нельзя: тогда кривая заходит под ФЛАГ.
   * Чтобы КОРЕШКИ при этом не попадали в незакрашенный клин, их ряд
   * сдвигается вправо на тот же размах — этим занимается вызывающий,
   * читая размах как CSS-переменную.
   */
  const L = box.leftW;
